  ...
  return <div className="site-shell">
    <header className="site-header"><div className="nav-wrap">
      ...
      <nav className={menuOpen ? 'nav-links open' : 'nav-links'} aria-label="Primary navigation">
        ...
        <div className="language-switcher" aria-label="Language selector">{languageOptions.map((option) => <button key={option.code} className={language === option.code ? 'active' : ''} onClick={() => updateLanguage(option.code)} aria-label={`Switch to ${option.code}`}>{option.label}</button>)}</div>
        <button className="theme-toggle" onClick={() => setDark(!dark)} aria-label={dark ? t.theme.light : t.theme.dark}>{dark ? '☼' : '☾'}</button>
      </nav>
    </div></header>

    <main>
      <section id="home" className="hero section-pad"><div className="hero-grid"><div className="hero-copy reveal"><p className="kicker"><span className="status-dot" /> {t.hero.availability}</p><h1>{t.hero.title} <em>{t.hero.titleAccent}</em></h1><p className="hero-summary">{t.hero.summary}</p><div className="hero-actions"><button className="button primary" onClick={() => navigate('projects')}>{t.hero.projects} <span>↗</span></button><a className="button secondary" href="/CV.pdf" download="Jean_Claude_Ishimwe_Resume.pdf">{t.hero.resume} <span>↓</span></a><button className="text-link" onClick={() => navigate('contact')}>{t.hero.contact} <span>→</span></button></div><div className="social-row">{socials.map((social) => <a key={social.label} href={social.href} target={social.href.startsWith('mailto') ? undefined : '_blank'} rel="noreferrer">{social.label}</a>)}</div></div><div className="hero-visual reveal"><div className="portrait-frame"><img src={profilePic} alt="Jean Claude Ishimwe" /></div></div></div><div className="scroll-cue">{t.hero.scroll} <span>↓</span></div></section>
      ...
      <section id="contact" className="section-pad contact-section"><div className="contact-grid"><div className="reveal"><SectionHeading eyebrow={t.contact.eyebrow} title={t.contact.title} intro={t.contact.intro} /><div className="contact-links"><a href={`mailto:${t.contact.email}`}>{t.contact.email} <span>↗</span></a><a href={socials[1].href} target="_blank" rel="noreferrer">{t.contact.linkedin} <span>↗</span></a></div></div><form className="contact-form reveal" onSubmit={submitContact}><label>{t.contact.name}<input name="name" required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder={t.contact.namePlaceholder} /></label><label>{t.contact.emailLabel}<input name="email" type="email" required value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder={t.contact.emailPlaceholder} /></label><label>{t.contact.message}<textarea name="message" required rows="5" value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} placeholder={t.contact.messagePlaceholder} /></label><button className="button primary" disabled={status === 'loading'}>{status === 'loading' ? t.contact.sending : t.contact.send}</button>{status === 'success' && <p className="form-status success">{t.contact.success}</p>}{status === 'error' && <p className="form-status error">{t.contact.error}</p>}</form></div></section>
    </main>
    <footer className="site-footer"><div><strong>Jean Claude Ishimwe<span>.</span></strong><p>{t.footer}</p></div><div className="footer-socials">{socials.map((social) => <a key={social.label} href={social.href}>{social.label}</a>)}</div><p>© {new Date().getFullYear()} Jean Claude Ishimwe</p></footer>
  </div>
}
